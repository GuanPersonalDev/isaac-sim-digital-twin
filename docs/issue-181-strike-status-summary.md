...
- ⚠️ **待處理／未解決（母球目標速度落差，暫不處理）**：母球實際速度
  ...
  ⚠️ **更正（2026-08-29）**：`#181` issue body 寫的「降至檔位 (c)」是
  舊版文字，`#183`（Milestone B fallback 階梯正式決策，2026-08-01
  定案）已明確**否決檔位 (c)（空揮到位+programmatic impulse），不再
  是可選的 fallback**——Milestone B 承諾線是檔位 (b)（方向+速度正確、
  中心擊球），若到 **B-CP3（2026-09-06，最後防線）**還是達不到，代價
  是「改砍 Block 12 深度換時間」，不是退回檔位 (c)。這個球速落差
  **不能靠降檔位解決**，只能從第十六節列出的 4 個技術選項（降
  `cue_ball_speed` 上界／進一步放寬姿態漂移／換 margin>0 的 roll
  候選／放寬 `_dof_limits`）裡選，需要使用者決定，這裡先記錄不處理，
  留到 B-CP3 前再定案。
...
## 結案：架構轉向 UR10e + 專用出力機構（2026-09-06）

上面「STRIKE 揮桿橫掃」與「AIM 終點 ≠ 揮桿起點」這兩個待處理項目最終沒有被
修掉——繼續往下查發現問題比表面看到的更深：把揮桿方向、對齊、後擺揉進同一組
加權多關節角速度解，在 UR3e 的幾何下是結構性卡住，不是參數沒調對。用真實
UR3e 幾何重新搜尋後證實：「純肘關節驅動」完全不可行（`axis_swing_alignment`
僅 0.074，近乎垂直）；放寬成「加權多關節驅動」後，`margin`（所需關節角速度／
馬達上限）與 `align`（球桿軸向與揮桿方向的平行度）之間存在結構性取捨——對應
manipulability ellipsoid：目標揮桿方向沒有對齊該姿態下橢球的長軸，兩個指標
無法同時達標，花費數小時調參搜尋都無法收斂。

參考業界機器人撞球專案的做法後，決定把「瞄準」跟「出力」拆開：手臂只負責
定位，實際擊球衝量改由末端一個專用線性致動器（PhysX PrismaticJoint + joint
drive）產生，不再依賴手臂關節角速度。手臂型號同時換成 UR10e（可達距離
1300mm、負載 12.5kg，RMPflow 官方支援清單有列），完整決策記錄與 11 項確認見
`C:\Users\Kuan\.claude\plans\ancient-skipping-wand.md`（計畫檔，非 repo 內
文件），逐步實作過程見 `docs/CHANGELOG.md` 各 UR10e 相關小節。

**這個轉向徹底繞開本文件記錄的全部問題**：揮桿方向永遠沿球桿軸（滑軌關節
本身的軸），對齊永遠是 1.0；後擺與揮桿起點的落差、揉合多個目標函式的零空間
問題都不存在，因為出力機構是 1 自由度、純線性。

### 驗收結果（`scripts/test_ur10e_table_flat.py`／`test_ur10e_table_bridge.py`）

| 案例 | 母球峰值速度 | 目標速度 | 達成率 | 球桿-母球碰撞 | 手臂本體碰撞 |
|---|---|---|---|---|---|
| flat | 1.8569 m/s | 1.995 m/s | 93.1% | 1 次 | 0 |
| bridge（高架橋） | 2.1757 m/s | 1.995 m/s | 109.1% | 1 次 | 0 |

對照：舊 UR3e 路徑受手臂關節角速度上限限制，`#176` 實測桿尖速度上限僅
1.313 m/s；UR10e＋線性致動器不受這個限制，兩個案例達成的母球速度都已超過
舊上限。

### B-CP3（#182）重複性驗證：同參數重跑 3 次，flat／bridge 皆逐位元一致

| 案例 | 3 次峰值速度 | 3 次達成率 | 3 次碰撞事件數 |
|---|---|---|---|
| flat | 1.8569 / 1.8569 / 1.8569 m/s | 93.1% / 93.1% / 93.1% | 1 / 1 / 1 |
| bridge | 2.1757 / 2.1757 / 2.1757 m/s | 109.1% / 109.1% / 109.1% | 1 / 1 / 1 |

headless 決定性模擬（無隨機性），3 次數值完全相同，滿足 B-CP3「同參數重複
3 次落點一致」的要求。

### 結論

#181（關節空間揮桿軌跡生成）、#182（真實接觸物理校正）、#183（Fallback
決策點總覽）以 superseded 結案——目標（手臂執行真實揮桿、球被桿尖真實打出去、
方向/速度與揮桿有清楚因果關係）已達成，但技術路徑改為 UR10e + 專用線性
出力機構，不是 #181 原始規劃的 UR3e 多關節聯合驅動；#182 列出的具體技術手段
（physics dt 1e-4／CCD／spin_efficiency）也沒有採用——新架構下桿尖速度直接
由滑軌關節線速度決定，dt=1/60 已足夠達到上述驗收標準，不需要那些補償手段。
Milestone B 落在 fallback (b) 檔位（方向+速度正確、真實桿尖接觸），未降級
到 (c)。
