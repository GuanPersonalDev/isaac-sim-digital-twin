...
| 2 | `shot_angle` | 擊球方向角 | **`[-180, 180)` 度**（`0°` 朝桌台 `+Y`，正對球堆；正角朝 `-X` 增加；#232 已復原整圈，見 [tech-design-232-shot-angle-restore.md](tech-design-232-shot-angle-restore.md)）。<br>2026-08-11 兩次修訂（#231）：① 由 `[0, 360)` 改成以 0° 為中心——舊區間的中點是 180°，等於未訓練的 policy 預設把母球往 kitchen 底庫打，且最佳方向 0° 落在正規化域邊界被不連續點切成兩半；② 曾權宜收窄到 ±30° 換取探索解析度（命中質量比 2.9% → 17.2%）。收窄不是設計結論；現行契約為整圈半開語意。`init_std` 重評與重訓見 #232-train |
...
| 出桿角度範圍 | **`[-180, 180)` 度**（#232 已復原整圈，見上方 Action 索引表第 2 列與 [tech-design-232-shot-angle-restore.md](tech-design-232-shot-angle-restore.md)；#231 曾權宜收窄為 −30 ~ +30。訓練超參重評見 #232-train） |
| 擊球位置偏移範圍 | ±0.5 球半徑 |
...
| # | 任務 | 預估 |
|---|---|---|
| 5-1 | 設計擊球參數資料格式（母球初速 0.65–3.3392m/s、角度 `[-180, 180)`（#232 已復原整圈；Milestone A 曾權宜 `[-30, 30]`）、位置偏移 ±0.5r）+ Unit Test | 0.5h |
