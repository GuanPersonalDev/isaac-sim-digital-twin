...
| 條件 | 分數 | 說明 |
|---|---|---|
| 散開程度 | 0.0 ~ 1.0 | 凸包面積×0.5（進袋球以袋口座標納入，維持 9 點不退化）+ 檯面上球平均最近鄰距離×0.5（進袋球排除），各自正規化 |
| 9號球進袋（白球未進） | +3.0 | 有效勝利 |
...
| # | 任務 | 預估 |
|---|---|---|
| 5-1 | 設計擊球參數資料格式（母球初速 0.65–3.3392m/s、角度 `[0, 360)`、位置偏移 ±0.5r）+ Unit Test | 0.5h |
