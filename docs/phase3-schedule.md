...
- **已合併／作廢的任務（不再列於下表）**
  ...
  - #233（逐球基座位置：Isaac Sim 場景碰撞檢查與真實 IK 驗證）→ 2026-08-23
    關閉時的驗收範圍（研究網格）跟真實 Kitchen 邊界不符，2026-08-26
    留言更正；真實邊界的完整 3×3 網格碰撞檢查已由 #181 承接完成（第
    十六節，6 個有效網格點全數無碰撞），維持關閉，不再獨立追蹤。
  - #90（UR5 的 RMPflow 設定檔）→ 2026-08-29 以 superseded 關閉：
    「非擊球移動」需求已由既有 `move_to_home()`（joint-space，不需
    RMPflow）滿足，且專案已全面轉往 Barrett WAM7，`UR5Robot` 未在任何
    正式路徑中被實際使用。
...
| 9-4 | Block 9 | 選定 RL 演算法（PPO）設定超參數【實際超出原估：含 reward 尺度分析與 RunPod 真實 PhysX 兩輪校準、MaskedPPO 無效步遮罩、1024-env 樣本量設計、PPO 接線／SPREAD_REF 自動驗證工具；最終 SPREAD_REF=0.0420、SPREAD_REWARD_SCALE=1.0】 | 0.5h | M5: RL 訓練與多環境 | TRUE | 2026-08-11 | 2026-08-09 | #123 |
...
| 8-3 | Block 8 | HUD 新增 ShotResult 顯示（散開分數白球狀態9號球狀態） | 0.5h | M4: 中途展示點 LinkedIn篇6 | TRUE | 2026-09-11 | 2026-09-06 | #116 |
| 6-6 | Block 6 | Debug Menu 新增「顯示當前 ShotResult」按鈕手動驗證計算正確性 | 0.5h | M3: 擊球動作與評估 | TRUE | 2026-09-11 | 2026-09-06 | #107 |
| 7-6 | Block 7 | Debug Menu 新增「印出當前 Observation」按鈕 | 0.5h | M3: 擊球動作與評估 | TRUE | 2026-09-11 | 2026-09-06 | #113 |
| 8-4 | Block 8 | 確認手動調整參數 → 擊球 → 結果顯示的完整流程 | 0.5h | M4: 中途展示點 LinkedIn篇6 | TRUE | 2026-09-11 | 2026-09-06 | #117 |
...
| 12-篇9 | Block 12 | LinkedIn 篇9（可選）——整合總結，內容視篇6/7/8 迴響決定，暫不展開細節 | 0h | M6: 整合測試與發布 LinkedIn篇8 | FALSE |  |  | #241 |
