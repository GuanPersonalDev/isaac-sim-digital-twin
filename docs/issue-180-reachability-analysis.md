...
1. ⚠️ **待處理／未解決（暫不處理）**：母球實際速度比需求速度低約
   ...
   **0.68 m/s**，全程 7 個關節裡有 5 個持續頂在 `_dof_limits` 的
   ...
   ⚠️ **更正（2026-08-29）**：`#181` issue body 寫的「若軌跡無法在擊球點
   達到目標速度向量 → 降至檔位 (c)」是舊版文字（2026-07-13 前後）。
   `#183`（Milestone B fallback 階梯正式決策，2026-08-01 定案）已明確
   **否決檔位 (c)（空揮到位+programmatic impulse），不再是可選的
   fallback**——Milestone B 承諾的完成線是檔位 (b)（方向+速度正確、
   中心擊球），若到 **B-CP3（2026-09-06，最後防線）**還是達不到目標
   速度，代價是「改砍 Block 12 深度換時間」，不是退回檔位 (c)。也就是
   這個球速落差**不能靠降檔位解決**，只能從第十六節列出的 4 個技術
   選項（降 `cue_ball_speed` 上界／進一步放寬姿態漂移／換 margin>0 的
   roll 候選／放寬 `_dof_limits`）裡選，需要使用者決定，這裡先記錄、
   暫不處理，留到 B-CP3 前再定案。
