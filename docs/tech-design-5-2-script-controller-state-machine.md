...
| `shot_angle` | `float` | RL 索引 2；**已復原整圈 `[-180, 180)` 度**（#232，見 [tech-design-232-shot-angle-restore.md](tech-design-232-shot-angle-restore.md)），`0°` 朝桌台 `+Y`（正對球堆），正角朝 `-X` 增加。2026-08-11 兩次修訂（#231）：先由 `[0, 360)` 改成以 0° 為中心，再權宜收窄 ±30°；現行契約為整圈半開語意 |
