...
def _wrap_angle(angle: float) -> float:
    ...
    以**區間中心**為錨，而不是下界。現行 `SHOT_ANGLE = (-180, 180)` 中心是 0，
    折回 `[-180, 180)`：兩個端點是同一個方向，`+180` 折成 `-180`，半開區間
    的語意成立。

    用下界當錨在收窄區間上會折錯邊（歷史 ±30 會變成 `[-30, 330)`，`-31` 變成
    `329`，離動作空間更遠而不是更近）。以中心為錨，整圈與收窄兩種寬度都正確
    ——這是錨點的理由，不是現行仍在收窄。

    ⚠️ 週期固定取 `360.0` 而**不是**區間寬度：寬度若當週期（歷史 ±30 時是 60），
    45° 會折成 -15°，完全不同的方向。圓的週期永遠是 360。

    折回後**不保證**落在 `[low, high]` 內（區間若不涵蓋整圈，有些物理角無法
    表達）。需要那個保證的呼叫端請用 `_canonical_shot_angle()`。
    """
...
def _canonical_shot_angle(angle: float) -> float:
    ...
    現行 `SHOT_ANGLE` 涵蓋整圈（#232），有限角折完都會落在區間內，這個檢查
    理論上不會觸發。仍保留為防禦：夾住等於把越界角謊報成端點方向。
    """
    low, high = ACTION_BOUNDS[_SHOT_ANGLE]
