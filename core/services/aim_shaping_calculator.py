...
#124 第一輪訓練（2026-08-11，it 238）的失敗模式**不是**「訊號太少」而是
「reward 地形是平的」：
...
policy 起點附近整片都是 -1.5，唯一的梯度是命中率約 4.5% 的那根尖刺。PPO 在
平原上只會縮變異數、往取樣雜訊剛好指的方向收斂——實測 238 個 iteration 後
`Policy/mean_std` 0.400 → 0.196、`Loss/learning_rate` 6.67e-4 → 2.3e-5，而
`Episode_Reward/spread` 與 `Episode_Termination/break_foul` **雙雙歸零**：
前者代表沒有任何一局合法接觸（1024 env 只要有一局，raw 值就會是 4.9e-5 而
不是 0.000000），後者代表也沒有碰到錯球——母球一顆球都沒碰到。

這一項把平原變成指向球堆的斜坡：越接近 1 號球給越多分，碰到就給滿分。
