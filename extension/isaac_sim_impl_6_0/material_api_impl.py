...
class MaterialAPIImpl(MaterialAPI):
    ...
    def apply_mdl_shader(
        ...
        shader.CreateInput("stripe_color", Sdf.ValueTypeNames.Color3f).Set(Gf.Vec3f(*stripe_color))
        shader.CreateInput("base_color", Sdf.ValueTypeNames.Color3f).Set(Gf.Vec3f(*base_color))
        material.CreateSurfaceOutput("mdl").ConnectToSource(shader.ConnectableAPI(), "surface")
        material.CreateDisplacementOutput("mdl").ConnectToSource(shader.ConnectableAPI(), "displacement")
        material.CreateVolumeOutput("mdl").ConnectToSource(shader.ConnectableAPI(), "volume")
        prim = stage.GetPrimAtPath(prim_path)
        UsdShade.MaterialBindingAPI(prim).Bind(material)
